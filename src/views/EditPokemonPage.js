...
class EditPokemonPage extends React.Component {
	...
	constructor (props) {
		...
		this.state = {
			...
			contributor: this.props.viewer.Pokemon ? this.props.viewer.Pokemon.contributor : '',
	...
	_updatePokemon = () => {
		Relay.Store.commitUpdate(
			new UpdatePokemonMutation({contributor: this.state.contributor, name: this.state.name, url: this.state.url, pokemonId: this.props.viewer.Pokemon.id, pokemon: null}),
			{
				onSuccess: () => { console.log('Drink updated', this.props.viewer.Pokemon.id);this.context.router.replace('/') },
				...
				<div className={classes.content}>
					<div style={{display: 'flex', flexDirection: 'row'}}>
						<PokemonCard
							...
							onContributorChange={(newContributor) => this.setState({contributor: newContributor})}
						/>
						{
						shouldPreviewRendered &&
							<PokemonCard
								...
								contributor={this.props.viewer.Pokemon.contributor}
							/>
						}
					</div>
